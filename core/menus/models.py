...
class Link(models.Model):
    ...
    """
    menu = models.ForeignKey(Menu, db_column='menu_id', 
                             related_name='links', verbose_name=_('menu'), on_delete=models.CASCADE)
    ...
    submenu = models.ForeignKey(Menu, db_column='submenu_id', related_name='parent_links',
                                blank=True, null=True, verbose_name=_('sub-menu'), on_delete=models.CASCADE)
...
class Bookmark(Link):
    ...
    def get_edit_url(self):
        ...
        return reverse('bookmark_edit', kwargs={'username': user.username, 'slug': self.slug})


    def get_delete_url(self):
        user = get_object_or_404(User, pk=self.slug.split('_')[1])
        return reverse('bookmark_delete', kwargs={'username': user.username, 'slug': self.slug})
